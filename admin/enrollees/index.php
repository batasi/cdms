        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT e.*, CONCAT(lastname, ', ', firstname,' ',middlename) as fullname, c.course 
						FROM `enrollee_list` e 
						INNER JOIN courses c ON e.package_id = c.id 
						WHERE e.status != 1  -- Exclude records with status = 1 (verified)
						ORDER BY e.`status` ASC, UNIX_TIMESTAMP(e.`date_created`)");
