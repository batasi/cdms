...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry_admin = $this->db->query("SELECT * FROM users where username = '".$username."' and password = '".md5($password)."' ");
		
		if($qry_admin->num_rows > 0){
			// Admin login successful
			foreach ($qry_admin->fetch_array() as $key => $value) {
		...
		$qry_student = $this->db->query("SELECT * FROM student where email = '".$username."' and id_no = '".$password."' ");
		if($qry_student->num_rows > 0){
			// Student login successful
			foreach ($qry_student->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
			}
			return 1;
		...
		return 3; // No matching records found in either table
	}
