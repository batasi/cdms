...
<!-- Navbar -->
      <nav class="main-header navbar navbar-expand navbar-light border  border-dark border-top-0  border-left-0 border-right-0 navbar-light text-sm shadow-sm">
        ...
        <ul class="navbar-nav">
          ...
          <li class="nav-item d-none d-sm-inline-block">
            <a href="..//../" class="nav-link"><b><?php echo (!isMobileDevice()) ? $_settings->info('name'):$_settings->info('short_name'); ?></b></a>
          </li>
        </ul>
        <!-- Right navbar links -->
          ...
          <li class="nav-item">
            <div class="btn-group nav-link">
                  <button type="button" class="btn btn-rounded badge badge-light dropdown-toggle dropdown-icon" data-toggle="dropdown">
                    ...
                    <span class="ml-3"><?php echo ucwords($_settings->userdata('firstname').' '.$_settings->userdata('lastname')) ?>
                    <?php if (isset($_SESSION['userdata']['sname']) && isset($_SESSION['userdata']['id_no'])): ?>                      
                      <?php echo $_SESSION['userdata']['sname']; ?>
                    <?php endif?>   
                  </span>
      ...
      <!-- /.navbar -->
      <script>
    // JavaScript to open the dropdown on click
